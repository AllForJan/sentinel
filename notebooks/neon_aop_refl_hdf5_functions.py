...
def h5refl2array(refl_filename):
  ...
  xMax = xMin + (refl_shape[1] * metadata['res']['pixelWidth']
                )  #xMax = left edge + (# of columns * resolution)",
  yMin = yMax - (refl_shape[0] * metadata['res']['pixelHeight']
...
def clean_band(bandArray, reflArray_metadata):
  ...
  band_clean = copy.copy(
      bandArray
  )  #make a copy of the array so you don't change the value of the original bandArray
...
def plot_band_array(band_array,
  ...
  plot = plt.imshow(
      band_array, extent=refl_extent, clim=colorlimit)
  if cbar == 'on':
    cbar = plt.colorbar(
        plot, aspect=40)
...
def calc_clip_index(clipExtent, h5Extent, xscale=1, yscale=1):
  ...
  ind_ext['yMax'] = round(h5rows -
                          (clipExtent['yMin'] - h5Extent['yMin']) / xscale)
  ind_ext['yMin'] = round(h5rows -
                          (clipExtent['yMax'] - h5Extent['yMin']) / yscale)
...
def subset_clean_refl(reflArray, reflArray_metadata, clipIndex):
  ...
  reflSubCleaned = reflArray[clipIndex['yMin']:clipIndex['yMax'], clipIndex[
      'xMin']:clipIndex['xMax'], :].astype(np.float)
  reflSubCleaned[reflSubCleaned == int(reflArray_metadata[
      'noDataVal'])] = np.nan
...
def stack_clean_bands(reflArray, reflArray_metadata, bands):
  ...
  stackedArray = np.zeros((reflArray.shape[0], reflArray.shape[1], len(bands)),
                          'uint8')  #pre-allocate stackedArray matrix

  for i in range(len(bands)):
    band_clean_names.append("b" + str(bands[i]) + "_refl_clean")
    band_clean_dict[band_clean_names[i]] = extract_clean_band(
        reflArray, reflArray_metadata, bands[i])
...
def subset_clean_band(reflArray, reflArray_metadata, clipIndex, bandIndex):
  ...
  bandCleaned = reflArray[clipIndex['yMin']:clipIndex['yMax'], clipIndex[
      'xMin']:clipIndex['xMax'], bandIndex - 1].astype(np.float)
  bandCleaned[bandCleaned == int(reflArray_metadata['noDataVal'])] = np.nan
  bandCleaned = bandCleaned / reflArray_metadata['scaleFactor']
...
def stack_subset_bands(reflArray, reflArray_metadata, bands, clipIndex):
  ...
  stackedArray = np.zeros((subArray_rows, subArray_cols, len(bands)),
                          'uint8')  #pre-allocate stackedArray matrix
  band_clean_dict = {}
  band_clean_names = []
